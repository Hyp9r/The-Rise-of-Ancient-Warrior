...
public class Map {
    ...
    /**Spawns*/
    /**Each map has it's own Pokemon spawns and each pokemon has a % chance to appear**/
    /**Common, uncommon, rare, ultra rare, epic, legendary**/

    /**Each map has range of levels for pokemons that spawn**/
    private int[] LevelRange = {1, 3};

    private List<Pokemon> pokemonSpawns = Arrays.asList(
            new Pokemon(1, 1, "Nidoran"),
            new Pokemon(1,1, "Pidgey"),
            new Pokemon(1,1,"Rattata"),
            new Pokemon(1,1, "Schyter")
    );
            ...
            for(int i=0; i<mapWidth / 16; i++){
                for(int j=0; j<mapHeight/16; j++){
                    ...
                    switch (tiles[i][j].getId()){
                        ...
                        case 37:
                        case 38:
                            tiles[i][j].setType(TileType.GRASS);
                            break;
                        default:
    ...
    public void draw(){
        ...
        for(int i=0; i<mapWidth / tileSize; i++){
            for(int j=0; j<mapHeight / tileSize; j++){
                batch.draw(tileRegions[tiles[i][j].getId()], i*16*scale, j*16*scale, tileSize*scale, tileSize*scale);
            }
        }
        batch.end();
        if(grid){
            ...
            for(int i=0; i<=mapHeight*tileSize; i+= tileSize){
                shapeRenderer.line(0, i, 40*tileSize, i);
            }
            for(int i=0; i<=mapWidth*tileSize; i+= tileSize){
                shapeRenderer.line(i, 0, i, 40*tileSize);
            }
            shapeRenderer.end();
        }
    }
