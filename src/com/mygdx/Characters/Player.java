...
import com.mygdx.Attacks.Attack;
...
public class Player {
    ...
    private void init(){
        ...
        animation[0] = animationUp;
        animation[1] = animationDown;
        animation[2] = animationRight;
        animation[3] = animationLeft;
    }
    ...
    public void draw(){
        ...
        if(state == StateTypes.IDLE)
            batch.draw(regions[getIdleSprite()], playerX, playerY, playerWidth, playerHeight);
        batch.end();
    ...
    public void playAnimation(){
        ...
        Gdx.gl.glBlendFunc(GL20.GL_SRC_ALPHA, GL20.GL_ONE_MINUS_SRC_ALPHA);
        batch.begin();
        batch.draw(animation[playerFacing].getKeyFrame(elapsed, true), playerX, playerY, playerWidth, playerHeight);
        batch.end();
        Gdx.gl.glDisable(GL20.GL_BLEND);
    }

    public void attack(){
        Attack atk = new Attack(10, 1);
        atk.draw(playerX, playerY);
    }
    ...
    public boolean isStandingOnGrass(){
        if(map.getTile(playerX, playerY).getType() == TileType.GRASS){
            System.out.println("Player is walking on grass");
            return true;
        }
        return false;
    }
    ...
    public void moveUp(){
        ...
        if (isColliding(playerX, playerY)) {
            //don't move you're coliding with an object
            playerY--;
        }else if(isTryingToSwim(playerX, playerY)){
            //you can't swim yet
            playerY--;
        }else{
            playerY++;
        ...
        /*if(this.state == StateTypes.WALKING){
            ...
            if(isColliding(playerX+5, playerY + 1)){
                ...
                BattleSystem.out.println("You can't swim yet!");
                playerY-=16;
            }else{
                playerY++;
